...
import com.lshwan.hof.domain.dto.member.SignupRequestDto;
...
public class MemberServiceImpl implements MemberService {
  ...
  public Long write(Member member) { 
    ...
    if (existingVerification.isPresent()) {
      ...
      if (verification.isVerified()) {  // 인증 여부를 확인
        log.info("이미 인증된 이메일입니다.");
        return savedMember.getMno();  // 인증된 상태로 바로 반환
      } else {
        verification.setVerificationCode(generateVerificationCode());
        verification.setExpiresAt(expiresAt);  // 만료시간 갱신
        verification.setVerified(false);  // 인증 상태를 다시 false로 설정
        emailVerificationRepository.save(verification);  // 갱신된 인증 정보 저장
      }
    } else {
      // 새로운 인증 정보 생성
      EmailVerification newEmailVerification = new EmailVerification();
      newEmailVerification.setEmail(email);  // 이메일 설정
      newEmailVerification.setVerificationCode(emailVerificationCode);
      newEmailVerification.setExpiresAt(expiresAt);
      newEmailVerification.setVerified(false);  // 초기 인증 상태는 false
      newEmailVerification.setMemberDetail(savedMember.getMemberDetail());  // MemberDetail 객체 설정
      emailVerificationRepository.save(newEmailVerification);  // 새 인증 정보 저장
    }
